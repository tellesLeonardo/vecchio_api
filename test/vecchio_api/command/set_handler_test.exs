defmodule VecchioApi.Command.SetHandlerTest do
  ...
  describe "parse/1" do
    ...
    test "parses SET command with key-value pair and handles mixed-case booleans as strings two test" do
      assert Handler.handle_command(~s{SET TRUE TRUE}) == {:error, "Value true is not valid as key"}
    end
    ...
    test "returns error for missing key or value in SET command" do
      assert Handler.handle_command("SET") == {:error, "set <chave> <valor> - Syntax error"}
      assert Handler.handle_command("SET my_key") == {:error, "set <chave> <valor> - Syntax error"}
    end
  end
  ...
  test "returns error for SET command with unmatched quote" do
    assert Handler.handle_command(~s{SET "AB"C}) == %VecchioApi.Command.Handler{code: :set, key: "\"AB\"", value: "C"}
  end
  ...
  test "new test nil" do
    assert Handler.handle_command("SET abc NIL") == {:error, "Nil value is not allowed."}
  end
